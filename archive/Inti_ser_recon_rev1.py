...
- modification code avec code de Doug & Andrew Smith pour acceleration stupefiante
    o vexctorisation dans calcul extraction des intensités pour reconstruction du disque
...
def UI_SerBrowse (WorkDir):
    ...
    [sg.Checkbox('Affiche disque noir sur image protuberances', default=False, key='-DISK-')],
    [sg.Checkbox('Sauve uniquement fits _recon et _clahe ', default=False, key='-SFIT-')],
    ...
    return FileNames, Shift, flag_display, ratio_fixe, disk_display, sfit_onlyfinal
...
if not(version_mac):
    ...
    serfiles, Shift, flag_display, ratio_fixe, disk_display,sfit_onlyfinal=UI_SerBrowse(WorkDir)
...
if len(serfiles)==1:
    tempo=40000 #tempo 40 secondes, pour no tempo mettre tempo=0 et faire enter pour terminer
else:
    tempo=1
...
for serfile in serfiles:
    ...
    frame, header, cercle=sol.solex_proc(serfile,Shift,flag_display,ratio_fixe,sfit_onlyfinal)
